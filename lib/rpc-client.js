...
RpcClient.prototype.notify = function(method, params) {
  ...
  if (u.isPlainObject(method) && method.hasOwnProperty('method') ||
    ...
    // add TTL, if it exists
    if (!!this._ttl) {
      notification.header = {};
      notification.header.ttl = this._ttl;
    }
  ...
  return new Promise(function(resolve, reject) {
    ...
    self._requests[correlator].timeoutId = setTimeout(function() {
      if (self._requests.hasOwnProperty(correlator)) {
        let error = new errors.RequestTimeoutError();
        error.request = request;
        self._requests[correlator].reject(error);
...
RpcClient.prototype._processMessage = function(message) {
  ...
  } else if (message.body.hasOwnProperty('responseCode') && message.body.responseCode === 'OK') {
    if (
      message.body.hasOwnProperty('responseMessage') &&
      message.body.responseMessage !== null &&
      message.body.responseMessage.hasOwnProperty('result')
    ) {
      request.resolve(message.body.responseMessage.result);
    } else {
      this._logger.warn({
        message: 'Reponse is empty',
